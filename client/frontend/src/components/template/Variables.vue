...
<template>
  <div class="space-y-6">
    <div class="text-sm text-muted-foreground" v-text="t('templates.description.Variables')" />
    <div v-if="template.groups && template.groups.length > 0" class="space-y-6">
      <div v-for="(group, index) in template.groups" :key="group.order" class="space-y-4">
        <div class="flex items-center justify-between gap-4 pb-3 border-b-2 border-border/50">
          <div class="flex-1 min-w-0">
            <h3 class="text-xl font-semibold text-foreground" v-text="group.display" />
            <div class="text-sm text-muted-foreground mt-1" v-text="group.description" />
          </div>
        ...
        <ul class="space-y-3">
          <li v-for="name in group.variables" :key="name" class="list-item cursor-pointer" @click="startEdit(name)">
            <div class="flex items-center justify-between gap-4">
              <span class="text-foreground flex-1" v-text="template.data[name].display" />
              <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <hr v-if="!isLastGroup(group.order) || grouplessVars.length > 0" class="border-t-2 border-border/50 my-6" />
      </div>
      <div v-if="grouplessVars.length > 0" class="space-y-4">
        <div class="flex items-center justify-between gap-4 pb-3 border-b-2 border-border/50">
          <h3 class="text-xl font-semibold text-foreground" v-text="t('templates.NoGroup')" />
          <btn variant="icon" @click="add()"><icon name="plus" /></btn>
        </div>
        <ul class="space-y-3">
          <li v-for="name in grouplessVars" :key="name" class="list-item cursor-pointer" @click="startEdit(name)">
            <div class="flex items-center justify-between gap-4">
              <span class="text-foreground flex-1" v-text="template.data[name].display" />
              <div class="flex items-center gap-2 flex-shrink-0">
    ...
    <div v-else class="space-y-4">
      <ul class="space-y-3">
        <li v-for="(item, name) in template.data" :key="name" class="list-item cursor-pointer" @click="startEdit(name)">
          <div class="flex items-center justify-between gap-4">
            <span class="text-foreground flex-1" v-text="item.display" />
            <btn variant="icon" @click.stop="remove(name)"><icon name="remove" /></btn>
          </div>
        </li>
      </ul>
    ...
    <overlay v-model="changeVarGroupOpen" :title="t('templates.variables.MoveToGroup')">
