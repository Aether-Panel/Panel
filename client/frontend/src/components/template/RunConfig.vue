...
<template>
  <div class="space-y-4">
    <ul v-if="Array.isArray(template.run.command)" class="space-y-3">
      <li v-for="(cmd, index) in template.run.command" :key="index" class="list-item cursor-pointer">
        <div class="flex items-center justify-between gap-4" @click="startEdit(index)">
          <div class="flex-1 min-w-0">
            <span class="block text-lg font-semibold text-foreground" v-text="cmd.command" />
            <span v-if="cmd.if" class="block text-sm text-muted-foreground mt-1" v-text="cmd.if" />
          </div>
