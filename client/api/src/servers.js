export class ServerApi {
  ...
  async getPlugins(id) {
    const res = await this._api.get(`/api/servers/${id}/plugins`)
    return res.data
  }

  async searchPlugins(id, query) {
    const res = await this._api.get(`/api/servers/${id}/plugins/search?q=${encodeURIComponent(query)}`)
    return res.data
  }

  async installPlugin(id, pluginId) {
    await this._api.post(`/api/servers/${id}/plugins/${pluginId}`)
    return true
  }

  async deletePlugin(id, pluginName) {
    await this._api.delete(`/api/servers/${id}/plugins`, { name: pluginName })
    return true
  }
...
class Server {
  ...
  async getPlugins(){
    return await this._api.server.getPlugins(this.id)
  }

  async searchPlugins(query){
    return await this._api.server.searchPlugins(this.id, query)
  }

  async installPlugin(pluginId){
    return await this._api.server.installPlugin(this.id, pluginId)
  }

  async deletePlugin(pluginName){
    return await this._api.server.deletePlugin(this.id, pluginName)
  }
