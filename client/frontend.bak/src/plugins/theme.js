...
import SkyPanelStyles from '@/themes/skypanel/theme.scss?inline'
import SkyPanelManifest from '@/themes/skypanel/manifest.json'
...
export const BUILTIN_THEME_DEFINITIONS = {
  ...
  SkyPanel: {
    styles: SkyPanelStyles,
    manifest: SkyPanelManifest
  }
}

export const BUILTIN_THEME_NAMES = Object.keys(BUILTIN_THEME_DEFINITIONS)
...
const availableThemes = [...BUILTIN_THEME_NAMES]
...
const themeApi = {
  ...
  setTheme: async (newTheme, settings = {}, save = true) => {
    ...
    const builtInTheme = BUILTIN_THEME_DEFINITIONS[newTheme]
    let manifest
    let themeCss = ''
    if (builtInTheme) {
      manifest = builtInTheme.manifest
      if (newTheme === 'Default') {
        themeCss = builtInTheme.styles
      } else if (manifest.keepDefaultCss) {
        themeCss = `${defaultStyles}\n${builtInTheme.styles}`
      } else {
        themeCss = builtInTheme.styles
      }
    } else {
    ...
    styles.textContent = themeCss
    sidebarClosedBelow.value = manifest.sidebarClosedBelow || 1200
    if (manifest.keepDefaultCss && newTheme !== 'Default') {
  ...
  getThemeSettings: async (theme = activeTheme.value) => {
    let definition
    if (BUILTIN_THEME_DEFINITIONS[theme]) {
      definition = {...BUILTIN_THEME_DEFINITIONS[theme].manifest.settings}
    } else {
...
export default (conf) => {
  return {
    install: (app) => {
      ...
      config.themes.active = config.themes.active === 'SkyPanel' ? 'Default' : config.themes.active
      config.themes.available = (config.themes.available || ['Default']).map(theme =>
        theme === 'SkyPanel' ? 'Default' : theme
      )
      ...
      config.themes.available
        .filter(e => BUILTIN_THEME_NAMES.indexOf(e) === -1)
        .map(e => availableThemes.push(e))
