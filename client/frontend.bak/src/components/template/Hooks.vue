...
<template>
  <div class="space-y-8">
    <div class="space-y-4">
      <h2 class="text-2xl font-bold text-foreground" v-text="t('templates.PreRunHook')" />
      <div class="text-sm text-muted-foreground" v-text="t('templates.description.PreRunHook')" />
      <operator-list v-model="template.run.pre" :add-label="t('templates.AddPreStep')" @update:modelValue="update()" />
    </div>
    <div class="space-y-4">
      <h2 class="text-2xl font-bold text-foreground" v-text="t('templates.PostRunHook')" />
      <div class="text-sm text-muted-foreground" v-text="t('templates.description.PostRunHook')" />
