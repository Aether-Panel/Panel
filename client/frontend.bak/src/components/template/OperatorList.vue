...
<template>
  <div class="space-y-4">
    <ul class="space-y-3">
      <li v-for="(step, index) in model" :key="index" class="list-item cursor-pointer">
        <div class="flex items-center justify-between gap-4" @click="startEdit(index)">
          <div class="flex-1 min-w-0">
            <span class="block text-lg font-semibold text-foreground" v-text="generateOperatorLabel(t, step)" />
            <span v-if="step.if" class="block text-sm text-muted-foreground mt-1" v-text="step.if" />
          </div>
