...
const envDefaults = {
  host: { type: 'host' },
  docker: { type: 'docker', image: 'SkyPanel/generic' }
}
...
<template>
  <div class="space-y-4">
    <ul class="space-y-3">
      <li v-for="(env, index) in template.supportedEnvironments" :key="env.type" class="list-item cursor-pointer">
        <div class="flex items-center justify-between gap-4" @click="editEnv(index)">
          <span class="text-foreground" v-text="t(`env.${env.type}.name`)" />
