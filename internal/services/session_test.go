...
func setupSessionTestDB(t *testing.T) *gorm.DB {
	...
	err = db.AutoMigrate(&models.User{}, &models.Role{}, &models.Permissions{}, &models.Client{}, &models.Server{}, &models.Session{}, &models.Node{})
	if !assert.NoError(t, err) {
		t.FailNow()
	}
...
func TestSession_ValidateExpiredReturnsError(t *testing.T) {
	...
	db.Model(&models.Session{}).Where("token = ?", func() string {
		h, _ := HashToken(token)
		return h
...
func TestSession_SlidingExpirationRenewsWhenHalfElapsed(t *testing.T) {
	...
	err = db.Model(&models.Session{}).Where("token = ?", hashed).Update("expiration_time", past).Error
	assert.NoError(t, err)
...
func TestSession_ValidateNode(t *testing.T) {
	...
	local, err := ss.ValidateNode(models.LocalNode.Secret)
	assert.NoError(t, err)
