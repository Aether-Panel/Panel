...
var dbObjects = []interface{}{
	...
	&domain.Database{},
	&domain.PanelSetting{},
	&domain.ExTransferSession{},
	&domain.ExTransferLog{},
	&domain.APIKey{},
	&domain.ProvisionProduct{},
}
		...
		},
		{
			...
			Migrate: func(db *gorm.DB) error {
				...
				updates := []struct {
					...
					{&domain.Database{}, "databases", "server_id"},
					{&domain.UptimeStatus{}, "uptime_statuses", "server_id"},
					{&domain.Backup{}, "backups", "server_id"},
					{&domain.Permissions{}, "permissions", "server_identifier"},
				}
	...
	{
		{
			...
			Migrate: func(_ *gorm.DB) error {
				...
				// err := db.Create(&domain.TemplateRepo{
				// 	Name:   "community",
		...
		},
		{
			...
			Migrate: func(db *gorm.DB) error {
				...
				err := db.AutoMigrate(&domain.Permissions{})
				if err != nil {
					return err
				}

				if !db.Migrator().HasColumn(&domain.Permissions{}, "admin") {
	...
	{
		{
			...
			Migrate: func(db *gorm.DB) error {
				...
				result := db.Where(domain.Role{Name: "Administrador"}).FirstOrCreate(adminRole)
				if result.Error != nil {
					return result.Error
				}
				...
				result = db.Where(domain.Role{Name: "Usuario"}).FirstOrCreate(userRole)
				return result.Error
			},
		},
		{
