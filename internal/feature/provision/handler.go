...
func provisionServer(c *gin.Context) {
	...
	// 3. Find Template
	ts := &template.TemplateRepo{DB: db}
	...
	// 7. Call Daemon
	serverCreation := &domain.ServerCreation{
...
func getServerByRequest(db *gorm.DB, req ProvisionActionRequest, ss *ServerRepo) (*domain.Server, error) {
	if req.ServiceID != "" {
...
func provisionTerminate(c *gin.Context) {
	...
	terminateOne := func(srv *domain.Server) {
		node := &srv.Node
