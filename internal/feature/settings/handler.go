...
func getSetting(c *gin.Context) {
	...
	for _, v := range editableStringEntries {
		if v.Key() == key {
			c.JSON(http.StatusOK, domain.Setting{Value: v.Value()})
			return
		}
	}
...
// @Param key path string true "The config key"
// @Param value body domain.Setting true "The new value for the setting"
// @Tags Panel Settings
...
func setSetting(c *gin.Context) {
	...
	if key == "panel.settings.masterNodeIp" {
		...
		err = db.Save(&domain.PanelSetting{
			Key:   "master_node_ip",
...
// @Failure 400 {object} ErrorResponse
// @Failure 500 {object} ErrorResponse
// @Param data body domain.ChangeMultipleSettings true "Config data to apply"
// @Tags Panel Settings
// @Router /api/settings [post]
// @Security OAuth2Application[settings.edit]
func setSettings(c *gin.Context) {
	var settings *domain.ChangeMultipleSettings
	var err error
