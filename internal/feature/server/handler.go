...
// @Success 200 {object} domain.ServerSearchResponse
...
func searchServers(c *gin.Context) {
	...
	if !isAdmin && username != "" && user.Username != username {
		c.JSON(http.StatusOK, &domain.ServerSearchResponse{
			Servers: []*domain.ServerView{},
	...
	results, total, err := ss.Search(ServerSearch{
		Username: username,
		NodeID:   uint(node),
		Name:     nameFilter,
...
// @Success 200 {object} domain.GetServerResponse
...
func getServer(c *gin.Context) {
	...
	var perms *domain.PermissionView
	...
	var serverView *domain.ServerView
	if isAdmin {
		serverView = domain.FromServer(server)
	} else {
		serverView = domain.RemoveServerPrivateInfo(domain.FromServer(server))
	}

	d := &domain.GetServerResponse{
		Server: serverView,
		Perms:  perms,
	}
...
// @Success 200 {object} domain.CreateServerResponse
// @Param id path string true "Server ID"
// @Param server body domain.ServerCreation true "Creation information"
// @Tags Daemon Servers
...
func createServer(c *gin.Context) {
	...
	postBody := &domain.ServerCreation{}
	...
	c.JSON(http.StatusOK, &domain.CreateServerResponse{ID: serverID})
}
...
// @Param id path string true "Server ID"
// @Param server body domain.ServerWithName true "Server definition"
// @Tags Daemon Servers
...
func editServer(c *gin.Context) {
	...
	postBody := &domain.ServerWithName{}
...
func deleteServer(c *gin.Context) {
	...
	if err := db.Model(&domain.Server{}).Where("parent_server_id = ?", server.Identifier).Count(&childrenCount).Error; err == nil && childrenCount > 0 {
	...
	if err := db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("server_identifier = ?", server.Identifier).Delete(&domain.Permissions{}).Error; err != nil {
			return err
		}

		var clientIDs []uint
		if err := tx.Model(&domain.Client{}).Where("server_id = ?", server.Identifier).Pluck("id", &clientIDs).Error; err == nil && len(clientIDs) > 0 {
			if err := tx.Where("client_id IN ?", clientIDs).Delete(&domain.Permissions{}).Error; err != nil {
				return err
			}
		}

		if err := tx.Where("server_id = ?", server.Identifier).Delete(&domain.UptimeStatus{}).Error; err != nil {
			return err
		}
		if err := tx.Where("server_id = ?", server.Identifier).Delete(&domain.Backup{}).Error; err != nil {
			return err
		}
		if err := tx.Where("server_id = ?", server.Identifier).Delete(&domain.Client{}).Error; err != nil {
			return err
		}
		if err := tx.Where("server_identifier = ?", server.Identifier).Delete(&domain.Session{}).Error; err != nil {
			return err
		}
...
// @Success 200 {object} []domain.UserPermissionsView
// @Param id path string true "Server ID"
...
func getServerUsers(c *gin.Context) {
	...
	users := map[*domain.User][]*scopes.Scope{}
	...
	data := make([]*domain.UserPermissionsView, 0)
	for k, v := range users {
		data = append(data, &domain.UserPermissionsView{
			Username: k.Username,
			Email:    k.Email,
			Scopes:   v,
		})
...
// @Param permissions body domain.PermissionView true "New permissions to apply"
// @Tags Daemon Servers
...
func editServerUser(c *gin.Context) {
	...
	if firstTimeAccess {
		es := emailService.GetEmailService()
...
// @Success 200 {object} domain.Backup
// @Param id path string true "Server ID"
// @Tags Daemon Servers
...
func createBackup(c *gin.Context) {
	...
	backup := &domain.Backup{Name: name, FileName: responseData.BackupFileName, ServerID: server.Identifier}
...
func proxyServerRequest(c *gin.Context) {
	...
	if c.IsWebsocket() {
		...
		permService := &permission.PermissionRepo{DB: db}
...
func proxyHTTPRequest(c *gin.Context, path string, ns *node.NodeRepo, node *domain.Node) {
	// If it's a local node, check if the server is a ghost (in DB but not on disk)
	if node.IsLocal() {
		serverID := c.Param("serverId")
		if runtime.GetFromCache(serverID) == nil {
...
func proxySocketRequest(c *gin.Context, path string, ns *node.NodeRepo, node *domain.Node) {
	if node.IsLocal() {
		serverID := c.Param("serverId")
...
func toggleServerSuspension(c *gin.Context) {
	...
	c.JSON(http.StatusOK, domain.FromServer(server))
}

func getServerFromGin(c *gin.Context) *domain.Server {
	return c.MustGet("server").(*domain.Server)
}

func checkGhost(s *domain.Server, v *domain.ServerView) {
	if s.Node.IsLocal() && runtime.GetFromCache(s.Identifier) == nil {
		v.IsGhost = true
	}
}
...
func getUserScopesForSearch(user *domain.User, ps *permission.PermissionRepo) []*scopes.Scope {
	userScopes := make([]*scopes.Scope, 0)
...
func processServerSearchResults(results []*domain.Server, isAdmin bool, user *domain.User, ps *permission.PermissionRepo, userScopes []*scopes.Scope) []*domain.ServerView {
	var data []*domain.ServerView
	if isAdmin {
		data = domain.FromServers(results)
	} else {
		data = domain.RemoveServerPrivateInfoFromAll(domain.FromServers(results))
	}
...
func getImpactedUsersByServerDeletion(db *gorm.DB, identifier string) ([]domain.User, error) {
	ps := permission.PermissionRepo{DB: db}
...
func shutdownServerOnNodeBeforeDeletion(ns *node.NodeRepo, node *domain.Node, identifier string) error {
...
func executeServerDeletionOnNode(c *gin.Context, ns *node.NodeRepo, node *domain.Node, identifier string) error {
...
func updateParentServerLimitsAfterDeletion(db *gorm.DB, ns *node.NodeRepo, node *domain.Node, server *domain.Server) {
	var parent domain.Server
...
func sendServerDeletionEmails(users []domain.User, server *domain.Server) {
	es := emailService.GetEmailService()
...
func validateAndBuildServerCreation(c *gin.Context, db *gorm.DB, us *user.UserRepo, postBody *domain.ServerCreation) (*domain.Server, []*domain.User, error) {
	if postBody.ParentServerID != nil && *postBody.ParentServerID != "" {
		...
		var parentPerms []domain.Permissions
	...
	users := make([]*domain.User, len(postBody.Users))
...
func checkParentServerLimits(tx *gorm.DB, server *domain.Server, parentAvailableCPU *int, parentAvailableMemory, parentAvailableDisk *int64) error {
	var parent domain.Server
...
func assignServerPermissions(ps *permission.PermissionRepo, serverID string, users []*domain.User) error {
	for _, v := range users {
...
func notifyNodeOfServerCreation(c *gin.Context, ns *node.NodeRepo, node *domain.Node, postBody *domain.ServerCreation, server *domain.Server) error {
	reader := &bytes.Buffer{}
...
func updateParentServerLimitsOnNode(c *gin.Context, db *gorm.DB, ns *node.NodeRepo, node *domain.Node, server *domain.Server, parentAvailableCPU int, parentAvailableMemory int64, parentAvailableDisk int64) {
	parentDataUpdate := map[string]interface{}{
...
func sendServerCreationEmails(users []*domain.User, server *domain.Server) {
	es := emailService.GetEmailService()
	for _, user := range users {
		err := es.SendEmail(user.Email, "addedToServer", map[string]interface{}{
			"Server":        server,
