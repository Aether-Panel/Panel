package node

import (
	"bytes"
	"errors"
	"fmt"
	"github.com/SkyPanel/SkyPanel/v3/internal/domain"
	"github.com/SkyPanel/SkyPanel/v3/internal/shared/config"
	"github.com/SkyPanel/SkyPanel/v3/internal/shared/logging"
	"github.com/SkyPanel/SkyPanel/v3/pkg/skypanel"
	"github.com/gorilla/websocket"
	"gorm.io/gorm"
	"io"
	"net/http"
	"net/http/httptest"
	"net/url"
	"strconv"
	"strings"
)

var wsupgrader = websocket.Upgrader{
	ReadBufferSize:  1024,
	WriteBufferSize: 1024,
	CheckOrigin: func(_ *http.Request) bool {
		return true
	},
}

func init() {
	SyncNodeToConfig()
}

func SyncNodeToConfig() {
	var masterURL = strings.TrimSuffix(strings.TrimPrefix(strings.TrimPrefix(config.MasterURL.Value(), "http://"), "https://"), "/")
	var masterParts = strings.SplitN(masterURL, ":", 2)
	domain.LocalNode.PublicHost = strings.Split(masterURL, ":")[0]
	domain.LocalNode.PrivateHost = strings.Split(masterURL, ":")[0]

	if len(masterParts) == 2 {
		port, err := strconv.ParseUint(masterParts[1], 10, 16)
		if err == nil {
			domain.LocalNode.PublicPort = uint16(port)
			domain.LocalNode.PrivatePort = uint16(port)
		}
	} else {
		// default port to 80 or 443 as the url doesn't have one, so we can assume one or other
		if strings.HasPrefix(config.MasterURL.Value(), "https://") {
			domain.LocalNode.PublicPort = 443
			domain.LocalNode.PrivatePort = 443
		} else {
			domain.LocalNode.PublicPort = 80
			domain.LocalNode.PrivatePort = 80
		}
	}

	sftpHost := config.SftpHost.Value()
	sftpParts := strings.SplitN(sftpHost, ":", 2)

	if len(sftpParts) == 2 {
		port, err := strconv.ParseUint(sftpParts[1], 10, 16)
		if err == nil {
			domain.LocalNode.SFTPPort = uint16(port)
		}
	}
}

type NodeRepo struct {
	DB *gorm.DB
}

func (ns *NodeRepo) GetAll() ([]*domain.Node, error) {
	var nodes []*domain.Node

	res := ns.DB.Find(&nodes)

	if res.Error != nil {
		return nil, res.Error
	}

	if config.PanelEnabled.Value() {
		// Sync the global Master Node IP from the DB
		var masterSetting domain.PanelSetting
		if err := ns.DB.Where("`key` = ?", "master_node_ip").First(&masterSetting).Error; err == nil && masterSetting.Value != "" {
			domain.LocalNode.PublicHost = masterSetting.Value
			domain.LocalNode.PrivateHost = masterSetting.Value
		}
		nodes = append(nodes, domain.LocalNode)
	}

	return nodes, nil
}

func (ns *NodeRepo) Get(id uint) (*domain.Node, error) {
	model := &domain.Node{}

	if id == domain.LocalNode.ID && config.PanelEnabled.Value() {
		var masterSetting domain.PanelSetting
		if err := ns.DB.Where("`key` = ?", "master_node_ip").First(&masterSetting).Error; err == nil && masterSetting.Value != "" {
			domain.LocalNode.PublicHost = masterSetting.Value
			domain.LocalNode.PrivateHost = masterSetting.Value
		}
		return domain.LocalNode, nil
	}

	res := ns.DB.First(model, id)
	return model, res.Error
}

func (ns *NodeRepo) Update(model *domain.Node) error {
	if model.ID == domain.LocalNode.ID && config.PanelEnabled.Value() {
		return nil
	}

	res := ns.DB.Save(model)
	return res.Error
}

func (ns *NodeRepo) Delete(id uint) error {
	if id == domain.LocalNode.ID && config.PanelEnabled.Value() {
		return errors.New("cannot delete local node")
	}

	model := &domain.Node{
		ID: id,
	}

	var count int64
	ns.DB.Model(&domain.Server{}).Where("node_id = ?", model.ID).Count(&count)
	if count > 0 {
		return skypanel.ErrNodeHasServers
	}

	res := ns.DB.Delete(model)
	return res.Error
}

func (ns *NodeRepo) Create(node *domain.Node) error {
	res := ns.DB.Create(node)
	return res.Error
}

func (ns *NodeRepo) CallNode(node *domain.Node, method string, path string, body io.ReadCloser, headers http.Header) (*http.Response, error) {
	var fullURL string
	var err error

	if node.IsLocal() {
		fullURL = "http://localhost" + path
	} else {
		fullURL, err = createNodeURL(node, path)
		if err != nil {
			return nil, err
		}
	}

	addr, err := url.Parse(fullURL)
	if err != nil {
		return nil, err
	}

	request := &http.Request{
		Method: method,
		URL:    addr,
		Header: headers,
	}

	if method != "GET" && body != nil {
		request.Body = body
	}

	ts, err := NewTokenService()
	if err != nil {
		return nil, err
	}

	// generate a new JWT token
	token, err := ts.GenerateRequest()
	if err != nil {
		return nil, err
	}

	if request.Header == nil {
		request.Header = http.Header{}
	}
	request.Header.Set("Authorization", "Bearer "+token)

	if node.IsLocal() {
		w := &httptest.ResponseRecorder{}
		w.Body = &bytes.Buffer{}
		skypanel.Engine.ServeHTTP(w, request)
		return w.Result(), err
	}

	response, err := skypanel.HTTP().Do(request)
	return response, err
}

func (ns *NodeRepo) OpenSocket(node *domain.Node, path string, writer http.ResponseWriter, request *http.Request) error {
	conn, err := wsupgrader.Upgrade(writer, request, nil)
	if err != nil {
		return err
	}

	ssl, err := doesDaemonUseSSL(node)
	if err != nil {
		return err
	}

	scheme := "ws"
	if ssl {
		scheme = "wss"
	}
	addr := fmt.Sprintf("%s:%d", node.PrivateHost, node.PrivatePort)

	u := fmt.Sprintf("%s://%s%s", scheme, addr, path)
	logging.Debug.Printf("Proxying connection to %s", u)

	ts, err := NewTokenService()
	if err != nil {
		return err
	}
	token, err := ts.GenerateRequest()
	if err != nil {
		return err
	}

	header := http.Header{}
	header.Set("Authorization", "Bearer "+token)

	c, _, err := websocket.DefaultDialer.Dial(u, header)
	if err != nil {
		// close the connection, because it failed
		_ = conn.Close()
		return err
	}

	go func(daemon *websocket.Conn, client *websocket.Conn) {
		defer func() {
			_ = daemon.Close()
			_ = client.Close()
		}()

		ch := make(chan error)
		go proxyRead(daemon, client, ch)

		err := <-ch

		if err != nil {
			logging.Error.Printf("Error proxying socket: %s", err)
		}
	}(c, conn)

	return nil
}

func doesDaemonUseSSL(node *domain.Node) (bool, error) {
	if node.IsLocal() {
		return false, nil
	}

	path := fmt.Sprintf("://%s:%d/daemon", node.PrivateHost, node.PrivatePort)

	// we want to do options so we can avoid auth
	u, err := url.Parse("https" + path)
	if err != nil {
		return false, err
	}

	request := &http.Request{Method: http.MethodOptions, URL: u}
	_, err = skypanel.HTTP().Do(request)

	if err != nil {
		u, err = url.Parse("http" + path)
		if err != nil {
			return false, err
		}

		request = &http.Request{Method: http.MethodOptions, URL: u}
		_, err = skypanel.HTTP().Do(request)
		return false, err
	}

	return true, nil
}

func createNodeURL(node *domain.Node, path string) (string, error) {
	ssl, err := doesDaemonUseSSL(node)
	if err != nil {
		return "", err
	}

	path = strings.TrimPrefix(path, "/")
	path = strings.TrimSuffix(path, "/")

	protocol := "http"
	if ssl {
		protocol = "https"
	}

	return fmt.Sprintf("%s://%s:%d/%s", protocol, node.PrivateHost, node.PrivatePort, path), nil
}

func proxyRead(source, dest *websocket.Conn, ch chan error) {
	for {
		messageType, data, err := source.ReadMessage()

		if err != nil {
			ch <- err
			return
		}
		err = dest.WriteMessage(messageType, data)
		if err != nil {
			ch <- err
			return
		}
	}
}
