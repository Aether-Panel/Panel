...
func CreateExTransfer(c *gin.Context) {
	...
	session := domain.ExTransferSession{
...
func validateExTransfer(c *gin.Context) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		var session domain.ExTransferSession
		...
		if session.Status != domain.StatusCreated && session.Status != domain.StatusFailed {
		...
		session.Status = domain.StatusValidated
...
func consumeExTransfer(c *gin.Context) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		session.Status = domain.StatusMigrating
		return tx.Save(&session).Error
	})
...
func downloadExTransfer(c *gin.Context) {
	...
	if session.Status != domain.StatusMigrating {
...
func StartDataPlaneMigration(session domain.ExTransferSession, db *gorm.DB) {
	...
	if err != nil || (res != nil && res.StatusCode != http.StatusNoContent && res.StatusCode != http.StatusOK) {
		...
		db.Model(&session).Update("status", domain.StatusFailed)
...
func performPullTransferAsync(server *domain.Server, originURL, token string, db *gorm.DB) {
