package user

import (
	"bytes"
	"crypto/rand"
	"encoding/base64"
	"errors"
	"image"
	"image/png"
	"math/big"
	"strings"

	"github.com/SkyPanel/SkyPanel/v3/internal/domain"
	"github.com/SkyPanel/SkyPanel/v3/internal/shared/config"
	"github.com/SkyPanel/SkyPanel/v3/pkg/skypanel"
	"github.com/pquerna/otp"
	"github.com/pquerna/otp/totp"
	"golang.org/x/crypto/bcrypt"
	"gorm.io/gorm"
)

type UserRepo struct {
	DB *gorm.DB
}

func (us *UserRepo) Get(username string) (*domain.User, error) {
	model := &domain.User{
		Username: username,
	}

	err := us.DB.Where(model).First(model).Error

	if err != nil {
		return nil, err
	}
	return model, nil
}

func (us *UserRepo) GetByID(id uint) (*domain.User, error) {
	model := &domain.User{
		ID: id,
	}

	err := us.DB.Where(model).First(model).Error

	if err != nil {
		return nil, err
	}
	return model, nil
}

func (us *UserRepo) ValidateLogin(email string, password string) (user *domain.User, otpNeeded bool, err error) {
	user = &domain.User{
		Email: email,
	}

	err = us.DB.Where(user).First(user).Error

	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return
	}

	if user.ID == 0 || errors.Is(err, gorm.ErrRecordNotFound) {
		err = skypanel.ErrInvalidCredentials
		return
	}

	if !us.IsValidCredentials(user, password) {
		err = skypanel.ErrInvalidCredentials
		return
	}

	if user.OtpActive {
		otpNeeded = true
		return
	}
	return
}

func (us *UserRepo) ValidOtp(email string, token string) (user *domain.User, err error) {
	user = &domain.User{
		Email: email,
	}

	err = us.DB.Where(user).First(user).Error

	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return
	}

	if user.ID == 0 || errors.Is(err, gorm.ErrRecordNotFound) {
		err = skypanel.ErrInvalidCredentials
		return
	}

	if !totp.Validate(token, user.OtpSecret) {
		// check recovery codes
		rc := &domain.RecoveryCode{
			UserID: user.ID,
		}
		_ = rc.SetCode(token)

		res := us.DB.Where(rc).Delete(rc)
		if res.Error != nil {
			err = res.Error
		}
		if res.RowsAffected == 0 {
			err = skypanel.ErrInvalidCredentials
		}
	}
	return
}

func (us *UserRepo) IsValidCredentials(user *domain.User, password string) bool {
	return bcrypt.CompareHashAndPassword([]byte(user.HashedPassword), []byte(password)) == nil
}

func (us *UserRepo) GetByEmail(email string) (*domain.User, error) {
	model := &domain.User{
		Email: email,
	}

	err := us.DB.Where(model).First(model).Error

	if err != nil {
		return nil, err
	}
	return model, nil
}

func (us *UserRepo) Update(model *domain.User) error {
	return us.DB.Save(model).Error
}

func (us *UserRepo) Delete(model *domain.User) (err error) {
	return us.DB.Transaction(func(tx *gorm.DB) error {
		tx.Delete(domain.Permissions{}, "user_id = ?", model.ID)
		tx.Delete(domain.Client{}, "user_id = ?", model.ID)
		tx.Delete(domain.Session{}, "user_id = ?", model.ID)
		tx.Delete(domain.User{}, "id = ?", model.ID)
		return nil
	})
}

func (us *UserRepo) Create(user *domain.User) error {
	return us.DB.Create(user).Error
}

func (us *UserRepo) ChangePassword(username string, newPass string) error {
	user, err := us.Get(username)

	if err != nil {
		return err
	}

	err = user.SetPassword(newPass)
	if err != nil {
		return err
	}
	return us.Update(user)
}

func (us *UserRepo) GetOtpStatus(userID uint) (enabled bool, err error) {
	user, err := us.GetByID(userID)
	if err != nil {
		return
	}

	enabled = user.OtpActive
	return
}

func (us *UserRepo) StartOtpEnroll(userID uint) (secret string, imgStr string, err error) {
	user, err := us.GetByID(userID)
	if err != nil {
		return
	}

	var key *otp.Key
	key, err = totp.Generate(totp.GenerateOpts{
		Issuer:      config.CompanyName.Value(),
		AccountName: user.Email,
	})
	if err != nil {
		return
	}

	user.OtpSecret = key.Secret()
	user.OtpActive = false
	err = us.Update(user)
	if err != nil {
		return
	}

	var buf bytes.Buffer
	var img image.Image
	img, err = key.Image(256, 256)
	if err != nil {
		return
	}
	_ = png.Encode(&buf, img)

	imgStr = "data:image/png;base64," + base64.StdEncoding.EncodeToString(buf.Bytes())
	secret = key.Secret()
	return
}

func (us *UserRepo) gerateOtpRecoveryCode(length int) (string, error) {
	const chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
	code := make([]byte, length)
	for i := range length {
		num, err := rand.Int(rand.Reader, big.NewInt(int64(len(chars))))
		if err != nil {
			return "", err
		}
		code[i] = chars[num.Int64()]
	}
	return string(code), nil
}

func (us *UserRepo) generateOtpRecoveryCodes(numCodes int) ([]string, error) {
	codes := make([]string, numCodes)
	for i := range numCodes {
		code, err := us.gerateOtpRecoveryCode(12)
		if err != nil {
			return nil, err
		}
		codes[i] = code
	}
	return codes, nil
}

func (us *UserRepo) ValidateOtpEnroll(userID uint, token string) ([]string, error) {
	user, err := us.GetByID(userID)
	if err != nil {
		return nil, err
	}

	if !totp.Validate(token, user.OtpSecret) {
		return nil, skypanel.ErrInvalidCredentials
	}

	codes, err := us.generateOtpRecoveryCodes(10)
	if err != nil {
		return nil, err
	}

	rcs := make([]domain.RecoveryCode, len(codes))
	for i, code := range codes {
		rcs[i] = domain.RecoveryCode{
			UserID: user.ID,
		}
		_ = rcs[i].SetCode(code)
	}

	err = us.DB.Create(&rcs).Error
	if err != nil {
		return nil, err
	}

	user.OtpActive = true
	return codes, us.Update(user)

}

func (us *UserRepo) RegenerateOtpRecoveryCodes(userID uint) ([]string, error) {
	user, err := us.GetByID(userID)
	if err != nil {
		return nil, err
	}

	codes, err := us.generateOtpRecoveryCodes(10)
	if err != nil {
		return nil, err
	}

	rcs := make([]domain.RecoveryCode, len(codes))
	for i, code := range codes {
		rcs[i] = domain.RecoveryCode{
			UserID: user.ID,
		}
		_ = rcs[i].SetCode(code)
	}

	err = us.DB.Transaction(func(tx *gorm.DB) error {
		err := tx.Delete(&domain.RecoveryCode{}, "user_id = ?", user.ID).Error
		if err != nil {
			return err
		}

		return tx.Create(&rcs).Error
	})

	return codes, err
}

func (us *UserRepo) DisableOtp(userID uint) error {
	user, err := us.GetByID(userID)
	if err != nil {
		return err
	}

	return us.DB.Transaction(func(tx *gorm.DB) error {
		err := tx.Delete(&domain.RecoveryCode{}, "user_id = ?", user.ID).Error
		if err != nil {
			return err
		}

		user.OtpSecret = ""
		user.OtpActive = false
		return tx.Save(user).Error
	})
}

func (us *UserRepo) Search(usernameFilter, emailFilter string, pageSize, page uint) ([]*domain.User, int64, error) {
	var users []*domain.User

	query := us.DB

	usernameFilter = strings.ReplaceAll(usernameFilter, "*", "%")
	emailFilter = strings.ReplaceAll(emailFilter, "*", "%")

	if usernameFilter != "" && usernameFilter != "%" {
		query = query.Where("username LIKE ?", usernameFilter)
	}

	if emailFilter != "" && emailFilter != "%" {
		query = query.Where("email LIKE ?", emailFilter)
	}

	var count int64
	err := query.Model(users).Count(&count).Error

	if err != nil {
		return nil, 0, err
	}

	res := query.Preload("Role").Preload("Permissions").Offset(int((page - 1) * pageSize)).Limit(int(pageSize)).Find(&users)

	return users, count, res.Error
}

func (us *UserRepo) IsSecurePassword(password string) error {
	// TODO: Change to use validator
	if len(password) < 8 {
		return skypanel.ErrFieldLength("password", 8, 72)
	}
	return nil
}
