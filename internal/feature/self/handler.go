...
// @Failure 404 {object} skypanel.ErrorResponse
// @Failure 500 {object} skypanel.ErrorResponse
// @Param user body domain.UserView true "User information"
// @Tags Self
// @Router /api/self [PUT]
...
func updateSelf(c *gin.Context) {
	...
	if oldEmail != "" {
		err := emailService.GetEmailService().SendEmail(oldEmail, "emailChanged", map[string]interface{}{
	...
	if passwordChanged {
		err := emailService.GetEmailService().SendEmail(user.Email, "passwordChanged", nil, true)
		if err != nil {
			logging.Error.Printf("Error sending email: %s\n", err)
		}
	}
...
func validateOtpEnroll(c *gin.Context) {
	...
	err = emailService.GetEmailService().SendEmail(user.Email, "otpEnabled", nil, true)
	if err != nil {
		logging.Error.Printf("Error sending email: %s\n", err)
	}
	c.JSON(http.StatusOK, gin.H{
...
func regenerateOtpRecoveryCodes(c *gin.Context) {
	...
	err = emailService.GetEmailService().SendEmail(user.Email, "recoveryCodesRegenerated", nil, true)
	if err != nil {
		logging.Error.Printf("Error sending email: %s\n", err)
	}
	c.JSON(http.StatusOK, gin.H{
...
func disableOtp(c *gin.Context) {
	...
	err = emailService.GetEmailService().SendEmail(user.Email, "otpDisabled", nil, true)
	if err != nil {
		logging.Error.Printf("Error sending email: %s\n", err)
	}
	c.Status(http.StatusNoContent)
...
// @Failure 404 {object} skypanel.ErrorResponse
// @Failure 500 {object} skypanel.ErrorResponse
// @Param client body domain.Client false "Information for the client to create"
// @Tags Self
...
func createPersonalOAuth2Client(c *gin.Context) {
	...
	err = emailService.GetEmailService().SendEmail(user.Email, "oauthCreated", nil, true)
	if err != nil {
		logging.Error.Printf("Error sending email: %s\n", err)
	}
...
func deletePersonalOAuth2Client(c *gin.Context) {
	...
	err = emailService.GetEmailService().SendEmail(user.Email, "oauthDeleted", nil, true)
	if err != nil {
		logging.Error.Printf("Error sending email: %s\n", err)
	}
	c.Status(http.StatusNoContent)
