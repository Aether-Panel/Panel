...
func (us *UptimeRepo) TrackStatus(serverID string, isRunning bool) error {
	...
	var currentStatus *domain.UptimeStatus
...
func (us *UptimeRepo) GetUptimeStats(serverID string, since time.Time) (uptimeSeconds, downtimeSeconds int64, uptimePercent float64, err error) {
	var results []struct {
		IsRunning bool
		Duration  int64
	}

	query := us.DB.Model(&domain.UptimeStatus{}).
	...
	var activeStatus *domain.UptimeStatus
...
func (us *UptimeRepo) GetRecentHistory(serverID string, limit int) ([]*domain.UptimeStatus, error) {
	var records []*domain.UptimeStatus
...
func (us *UptimeRepo) GetAllServerUptime(since time.Time) (map[string]map[string]interface{}, error) {
	var results []struct {
		ServerID         string
		IsRunning        bool
		TotalDuration    int64
	...
	var activeStatuses []domain.UptimeStatus
	err = us.DB.Where("end_time IS NULL").Find(&activeStatuses).Error
	if err != nil {
		return nil, err
	}

	activeMap := make(map[string]*domain.UptimeStatus)
