...
func (ds *DatabaseRepo) GetAllForServer(serverID string) ([]*domain.Database, error) {
...
func (ds *DatabaseRepo) Get(id uint) (*domain.Database, error) {
	model := &domain.Database{}

	res := ds.DB.Preload("DatabaseHost").First(model, id)
	return model, res.Error
}

func (ds *DatabaseRepo) Delete(id uint) error {
	model, err := ds.Get(id)
	if err != nil {
		return err
	}
	...
	res := ds.DB.Delete(&domain.Database{}, id)
	return res.Error
}

func (ds *DatabaseRepo) Create(database *domain.Database) error {
	// Obtener el database host
	host := &domain.DatabaseHost{}
...
func (ds *DatabaseRepo) createInMySQL(database *domain.Database, host *domain.DatabaseHost) error {
...
func (ds *DatabaseRepo) deleteFromMySQL(database *domain.Database) error {
